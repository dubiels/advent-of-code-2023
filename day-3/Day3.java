...
public class Day3 {
            ...
            while (line != null) {
                ...
                if (i <= 139 && Character.isDigit(line.charAt(i))) {
                    ...
                    if ((indexes.get(indexes.size() - 1) + 1) <= 139) {
                        if (!Character.isLetterOrDigit(line.charAt(indexes.get(indexes.size() - 1) + 1))
                                && ((line.charAt(indexes.get(indexes.size() - 1) + 1)) != '.')) {
                            adjacent = true;
                        }
                    }
                    // Checks spot before the beginning of the number:
                    if ((indexes.get(0) != 0)) {
                        if (!Character.isLetterOrDigit(line.charAt(indexes.get(0) - 1))
                                && (line.charAt(indexes.get(0) - 1) != '.')) {
                            adjacent = true;                        }
                    }
                    ...
                    if (previousLine != null) {
                        for (int z = 0; z < indexes.size(); z++) {
                            if ((!Character.isLetterOrDigit(previousLine.charAt(indexes.get(z))))
                                    && (previousLine.charAt(indexes.get(z)) != '.')) {
                                adjacent = true;
                            }
                            // Diagonal cases:
                            if (indexes.get(0) - 1 > 0) {
                                if (((!Character.isLetterOrDigit(previousLine.charAt(indexes.get(0) - 1)))
                                        && (previousLine.charAt(indexes.get(0) - 1) != '.'))) {
                                    adjacent = true;
                                }
                            }
                            if (indexes.get(indexes.size() - 1) + 1 < 139) {
                                if (!Character
                                        .isLetterOrDigit(previousLine.charAt(indexes.get(indexes.size() - 1) + 1))
                                        && (previousLine.charAt(indexes.get(indexes.size() - 1) + 1) != '.')) {
                                    adjacent = true;
                                }
                            }
                        }
                    ...
                    if (nextLine != null) {
                        for (int z = 0; z < indexes.size(); z++) {
                            if ((!Character.isLetterOrDigit(nextLine.charAt(indexes.get(z))))
                                    && (nextLine.charAt(indexes.get(z)) != '.')) {
                                adjacent = true;

                            }
                            // Diagonal cases:
                            if (indexes.get(0) - 1 > 0) {
                                if (((!Character.isLetterOrDigit(nextLine.charAt(indexes.get(0) - 1)))
                                        && (nextLine.charAt(indexes.get(0) - 1) != '.'))) {
                                    adjacent = true;
                                }
                            }
                            if (indexes.get(indexes.size() - 1) + 1 < 139) {
                                if (!(Character
                                        .isLetterOrDigit(nextLine.charAt(indexes.get(indexes.size() - 1) + 1)))
                                        && (nextLine.charAt(indexes.get(indexes.size() - 1) + 1) != '.')) {
                                    adjacent = true;
                                }
                            }
                        }
                    ...
                    if (adjacent) {
                        for (int w = 0; w < indexes.size(); w++) {
                            current = current + Character.getNumericValue(line.charAt(indexes.get(w)))
                                    * (int) (Math.pow(10, (indexes.size() - w - 1)));
                        }
                        total += current;
    ...
    public static int numberDetector(int index, String line) {
        ...
        for (int w = 0; w < numberConstruction.size(); w++) {
            number = number + Character.getNumericValue(numberConstruction.get(w))
                    * (int) (Math.pow(10, (numberConstruction.size() - w - 1)));
